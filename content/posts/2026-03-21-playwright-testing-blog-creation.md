...
## 기능이 하나씩 붙자 마음이 불안해졌다.

PM으로 제품을 만들어가며, 기능이 하나씩 붙다보면 다른 기능들이 영향을 받고 내가 모르는 사이 여러 문제가 터지게 된 적이 한두번이 아니다.

역시나 블로그에도 기능이 하나둘 붙으면서 슬슬 **불안해지기 시작했다**.

글 쓰기 페이지에서 저장 누르면 편집 페이지로 잘 넘어가나? 태그 필터는 아직도 되나? 댓글 컴포넌트 안 깨졌나? 새 기능 하나 추가할 때마다 이전에 만든 것들을 일일이 클릭해서 확인하는 게 일이 됐다.

처음엔 그냥 로컬에서 띄워서 브라우저 열고 직접 눌러봤다. 근데 이게 매번 하려니까 귀찮은 정도를 넘어서 중요한 기능 테스트가 점점 잊혀지고, **빠뜨리게 된다**. *"아 이건 지난번에 됐으니까 괜찮겠지"* 하고 넘어가는 순간, 그게 깨져 있다.
...
위 플로우를 Playwright 를 활용해 테스트를 돌리면 이런 일이 벌어진다:
...
이걸 내가 매번 할 필요가 없고, **코드 한 줄이면 전부 돌아간다.**
...
이게 되니까 마음이 좀 놓인다. 새 기능을 추가하고 Playwright 으로 테스트를 돌렸을 때 **문제가 없다면 적어도 기존에 만든 것들은 안 깨졌다는 뜻이니까** 테스트한답시고 내가 일일이 눌러볼 필요가 없다.

난 내가 방금 수정한 기능이 잘 동작하는지만 확인하면 된다. (이전에는 방금 수정하지 않은 것들까지도 다 테스트해야 했던 반면에)
...
<div class="takeaway-box">만드는 것보다 <strong>이미 만든 걸 안 깨뜨리는 게</strong> 더 어렵다. 오늘부터는 그걸 로봇한테 맡기는 거다.</div>
