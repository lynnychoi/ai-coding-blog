...
## Server Component 삽질 (어제 이어서)

어제부터 이어진 것: Next.js App Router에서 서버 컴포넌트에 `onClick`을 쓰면 빌드가 터진다.
...
1. 파일 맨 위에 `"use client"` 선언 → 클라이언트 컴포넌트로 전환
2. 이벤트 핸들러가 필요한 부분만 별도 클라이언트 컴포넌트로 분리

오늘 태그 필터 기능을 만들 때도 이 패턴을 써야 했다.
홈페이지(`page.tsx`)는 서버 컴포넌트로 유지하고,
상태가 필요한 필터 UI만 `PostList.tsx`로 빼서 클라이언트 컴포넌트로 만들었다.

```
app/
  page.tsx          → 서버 컴포넌트 (데이터 fetch)
  components/
    PostList.tsx    → 클라이언트 컴포넌트 (태그 필터 state)
    Comments.tsx    → 클라이언트 컴포넌트 (댓글 form state)
```
